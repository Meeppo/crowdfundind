...
public class WorkflowTraceService extends BaseService {
	 ...
	 * @return
	 */
	public Page<Task> findTodoTasks(Page<Task> page,Task task,com.thinkgem.jeesite.modules.sys.entity.User user) {
		...
		if(task != null && StringUtils.isNotBlank(task.getName())){
			todoQuery = todoQuery.taskName(task.getName());
		}
			//当前用户的任务列表
		List<Task> todoList = todoQuery.taskAssignee(ObjectUtils.toString(user.getId())).list();
			//当前为被认领的任务列表
		List<Task> unsignedTasks = todoQuery.taskCandidateUser(ObjectUtils.toString(user.getId())).list();
			tasks.addAll(todoList);
			tasks.addAll(unsignedTasks);

		page.setCount(tasks.size());
		page.setList(tasks.subList(page.getFirstResult(),page.getLastResult()));
		return page;
	}
	/**
	 * 获取当前用户所有未完成任务
	 ...
	 * @return
	 */
	public Page<Task> findTodoTasks(Page<Task> page,Task task) {
		com.thinkgem.jeesite.modules.sys.entity.User user = UserUtils.getUser();
		return findTodoTasks(page, task, user);
	}
